...
class Handelinputdata():
	def __init__(self):
		...
		self.cleanhtml = self.getWebPage()

	def handel(self,team):
		data = self.getInputData(team)
		preProcessedInputData = self.preProcessInputData(data)
		v = VerifyData(preProcessedInputData,team)
		if v.verify():
	...
	def getInputData(self,team):		
		teamCharNum = self.cleanhtml.find(team)
		bloksize = self.findBlokSize(teamCharNum,self.cleanhtml)
		blok = self.cleanhtml[teamCharNum:teamCharNum+bloksize]
...
class VerifyData():
	def __init__(self,preProcessedInputData,team):
